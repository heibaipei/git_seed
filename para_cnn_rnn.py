...
dataset_dir = "G:/output/parallel_cnn_rnn/raw_data/"

with open(dataset_dir+"1_3_shuffle_cnn_dataset.pkl", "rb") as fp:
  	cnn_datasets = pickle.load(fp)
with open(dataset_dir+"1_3_shuffle_rnn_dataset.pkl", "rb") as fp:
  	rnn_datasets = pickle.load(fp)
with open(dataset_dir+"1_3_shuffle_labels.pkl", "rb") as fp:
...
print("shape of train:",np.shape(cnn_train_x))
...
print("shape of cnn_out before:",np.shape(cnn_out))
cnn_out=tf.transpose(cnn_out, [1, 0, 2])
print("shape after transpose:",np.shape(cnn_out))
cnn_out = tf.unstack(cnn_out)
print("shape of cnn_out:",np.shape(cnn_out))
cnn_out_fuse = cnn_out[0]
print("cnn_out_fuse:",cnn_out_fuse)
print("shape of cnn_out_fuse:",np.shape(cnn_out_fuse))
print("length of cnn_out:",len(cnn_out))
...
with tf.Session(config=config) as session:
	...
	print("test_accuracy shape:",np.shape(test_accuracy))
	print("test_loss shape:",np.shape(test_loss))
	print("test_pred shape:",np.shape(test_pred))
	print("test_true shape:",np.shape(test_true))
	print("test_posi shape:",np.shape(test_posi))
	print("test_pred_1_hot shape:",np.shape(test_pred_1_hot))
	print("test_true_list shape:",np.shape(test_true_list))
	...
	print("axis=0:",np.sum(confusion_matrix,axis=0))
	print("axis=1:",np.sum(confusion_matrix,axis=1))
	print("all:",np.sum(confusion_matrix))
	...
	# save result
	os.system("mkdir -p result\\cnn_rnn_parallel\\tune_rnn_layer\\"+output_dir)
	...
	writer = pd.ExcelWriter("C:\\Users\\Young\\PycharmProjects\\bci_first-master\\result\\cnn_rnn_parallel\\tune_rnn_layer\\"+output_dir+"\\"+output_file+".xlsx")
	...
	for key in one_hot_labels:
		...
		i += 1
	# writer.save()
	# with open("C:\\Users\\Young\\PycharmProjects\\bci_first-master\\result\\cnn_rnn_parallel\\tune_rnn_layer\\"+output_dir+"\\confusion_matrix.pkl", "wb") as fp:
  	# 	pickle.dump(confusion_matrix, fp)
	# save model
	for variable in tf.trainable_variables():
		print(variable.get_shape())
	saver = tf.train.Saver()
	saver.save(session, "C:\\Users\\Young\\PycharmProjects\\bci_first-master\\result\\cnn_rnn_parallel\\tune_rnn_layer\\"+output_dir+"\\model_"+output_file)
